...
import { packageSpecFor, runHarness } from './execution.js'
...
export async function runApp(options, io = {}) {
  ...
  if (options.command === 'doctor') return runDoctor(options, { stdout, stderr })
  ...
  for (const moduleId of selected) {
    ...
    const spec = options.command === 'install'
      ? packageSpecFor(moduleId, { localRoot, version: VERSION })
      : module.packageName
...
function runDoctor(options, { stdout }) {
  const rows = []
  const nodeVersion = process.versions.node
  rows.push({ ok: supportsNode(nodeVersion), label: 'Node.js', detail: nodeVersion })
  rows.push({ ok: commandExists(process.platform === 'win32' ? 'pnpm.cmd' : 'pnpm'), label: 'pnpm', detail: 'Harness profile plugin manager' })
  rows.push({ ok: commandExists(process.platform === 'win32' ? 'npx.cmd' : 'npx'), label: 'npx', detail: 'Playwright MCP launcher' })
  if (options.harness !== undefined) {
    ...
    rows.push({ ok: commandExists(process.platform === 'win32' ? 'dsh.cmd' : 'dsh'), label: 'dsh', detail: 'CLI on PATH' })
  }
